...
impl AD5675 {
    ...
    pub fn write_dac(&self, channel: u8, value: u16) -> Result<(), LinuxI2CError> {
        ...
        unsafe { ioctl(fd, (I2C_SLAVE as c_ulong).try_into().unwrap(), 0x77) };
        i2c_linux_sys::i2c_smbus_write_byte(fd, 0x04);
        unsafe {
            ioctl(
                fd,
        ...
        i2c_linux_sys::i2c_smbus_write_word_data(fd, 0x30 + channel, buffer);

        Ok(())
    }

    pub fn write_dac_i2cdev(&self, channel:u8, value: u16) -> Result<(), LinuxI2CError> {
