...
impl INA219 {
    ...
    pub fn configure(&self) -> Result<(), LinuxI2CError> {
        ...
        let config = CONFIG_BRNG_32 | CONFIG_PG_320 | CONFIG_BADC_16S | CONFIG_SADC_16S | CONFIG_MODE_SBT;
    ...
    pub fn read_data(&self) -> Result<(f32, f32, f32), LinuxI2CError> {
        ...
        let shunt_voltage = self.read_shunt_voltage(&mut dev).expect("cannot read shunt voltage on INA219");
        let bus_voltage = self.read_bus_voltage(&mut dev).expect("cannot read bus voltage on INA219");

        let (c_lsb, p_lsb) = self.calibrate(&mut dev).expect("cannot calibrate INA219");

        let current = self.read_current(&mut dev, c_lsb).expect("cannot read current on INA219");
        let power = self.read_power(&mut dev, p_lsb).expect("cannot read power on INA219");
    ...
    fn read_bus_voltage(&self, dev: &mut LinuxI2CDevice) -> Result<(f32), LinuxI2CError> {
        ...
        let bus_voltage_adc = (((bus_voltage_raw[0] as u16) << 8) | (bus_voltage_raw[1] as u16)) >> 3 & 0x1FFF;
