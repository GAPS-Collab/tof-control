...
impl RBInfluxDB {
    pub fn new(bucket: String) -> Self {
        let rb_name = gethostname().into_string().expect("cannot convert hostname").replace("tof-rb", "RB");
    ...
    //     bucket
    // }
    pub async fn rb_influxdb_write(&self, rat_id: &str, test: String) -> Result<(), Box<dyn std::error::Error>> {
        let hostname = gethostname().into_string().expect("cannot convert hostname");
        
        let client = influxdb2::Client::new(self.url, self.org, self.token);

        let mut rb_info_points = Self::rb_info_influxdb(&hostname, &rat_id, &test).expect("cannot get RB info InfluxDB data points");
        let mut rb_temp_points = Self::rb_temp_influxdb(&hostname, &rat_id, &test).expect("cannot get RB temperature InfluxDB data points");
        let mut rb_ph_points = Self::rb_ph_influxdb(&hostname, &rat_id, &test).expect("cannot get RB pressure & humidity InfluxDB data points");
        let mut rb_mag_points = Self::rb_mag_influxdb(&hostname, &rat_id, &test).expect("cannot get RB magnetic field InfluxDB data points");
        let mut rb_vcp_points = Self::rb_vcp_influxdb(&hostname, &rat_id, &test).expect("cannot get RB vcp InfluxDB data points");
    ...
    fn rb_info_influxdb(hostname: &str, rat_id: &str, test: &str) -> Result<Vec<DataPoint>, Box<dyn std::error::Error>> {
    ...
    fn rb_temp_influxdb(hostname: &str, rat_id: &str, test: &str) -> Result<Vec<DataPoint>, Box<dyn std::error::Error>> {
    ...
    fn rb_ph_influxdb(hostname: &str, rat_id: &str, test: &str) -> Result<Vec<DataPoint>, Box<dyn std::error::Error>> {
    ...
    fn rb_mag_influxdb(hostname: &str, rat_id: &str, test: &str) -> Result<Vec<DataPoint>, Box<dyn std::error::Error>> {
    ...
    fn rb_vcp_influxdb(hostname: &str, rat_id: &str, test: &str) -> Result<Vec<DataPoint>, Box<dyn std::error::Error>> {
