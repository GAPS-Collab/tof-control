...
use tof_control::device::{
    ad5675, bme280, cy8c9560a, ina226, lis3mdltr, max11645, pca9548a, si5345b, tmp112,
};
...
fn main() {
    ...
    println!(
        "Checking I2C Multiplexer 1 ({:#02X})",
        RB_PCA9548A_ADDRESS_1
    );
    ...
    let bme280 = match bme280 {
        Ok(_f32) => println!(
            "Passed => Channel  {}: BME280 ({:#02X})",
            RB_BME280_CHANNEL, RB_BME280_ADDRESS
        ),
        Err(_LinuxI2CError) => println!(
            "\x1b[91mFailed\x1b[0m => Channel  {}: BME280 ({:#02X})",
            RB_BME280_CHANNEL, RB_BME280_ADDRESS
        ),
    };
    ...
    let lis3mdltr = match lis3mdltr {
        Ok(_u8) => println!(
            "Passed => Channel  {}: LIS3MDLTR ({:#02X})",
            RB_LIS3MDLTR_CHANNEL, RB_LIS3MDLTR_ADDRESS
        ),
        Err(_LinuxI2CError) => println!(
            "\x1b[91mFailed\x1b[0m => Channel  {}: LIS3MDLTR ({:#02X})",
            RB_LIS3MDLTR_CHANNEL, RB_LIS3MDLTR_ADDRESS
        ),
    };
    ...
    let drs_tmp112 = match drs_tmp112 {
        Ok(_f32) => println!(
            "Passed => Channel  {}: DRS4 TMP112 ({:#02X})",
            RB_DRS_TMP112_CHANNEL, RB_DRS_TMP112_ADDRESS
        ),
        Err(_LinuxI2CError) => println!(
            "\x1b[91mFailed\x1b[0m => Channel  {}: DRS4 TMP112 ({:#02X})",
            RB_DRS_TMP112_CHANNEL, RB_DRS_TMP112_ADDRESS
        ),
    };
    ...
    let drs_dvdd_ina226 = match drs_dvdd_ina226 {
        Ok(_f32) => println!(
            "Passed => Channel  {}: DRS4 DVDD INA226 ({:#02X})",
            RB_DRS_DVDD_INA226_CHANNEL, RB_DRS_DVDD_INA226_ADDRESS
        ),
        Err(_LinuxI2CError) => println!(
            "\x1b[91mFailed\x1b[0m => Channel  {}: DRS4 DVDD INA226 ({:#02X})",
            RB_DRS_DVDD_INA226_CHANNEL, RB_DRS_DVDD_INA226_ADDRESS
        ),
    };
    ...
    let p3v3_ina226 = match p3v3_ina226 {
        Ok(_f32) => println!(
            "Passed => Channel  {}: 3.3V INA226 ({:#02X})",
            RB_P3V3_INA226_CHANNEL, RB_P3V3_INA226_ADDRESS
        ),
        Err(_LinuxI2CError) => println!(
            "\x1b[91mFailed\x1b[0m => Channel  {}: 3.3V INA226 ({:#02X})",
            RB_P3V3_INA226_CHANNEL, RB_P3V3_INA226_ADDRESS
        ),
    };
    ...
    let zynq_ina226 = match zynq_ina226 {
        Ok(_f32) => println!(
            "Passed => Channel  {}: ZYNQ INA226 ({:#02X})",
            RB_ZYNQ_INA226_CHANNEL, RB_ZYNQ_INA226_ADDRESS
        ),
        Err(_LinuxI2CError) => println!(
            "\x1b[91mFailed\x1b[0m => Channel  {}: ZYNQ INA226 ({:#02X})",
            RB_ZYNQ_INA226_CHANNEL, RB_ZYNQ_INA226_ADDRESS
        ),
    };
    ...
    i2c_mux_1
        ...
        .expect("cannot accesss to PCA9548A");
    let max11645 =
        max11645::MAX11645::new(I2C_BUS, RB_MAX11645_ADDRESS).read(RB_N1V5_CURRENT_INA200_CHANNEL);
    let max11645 = match max11645 {
        Ok(_f32) => println!(
            "Passed => Channel  {}: MAX11645 ({:#02X})",
            RB_MAX11645_CHANNEL, RB_MAX11645_ADDRESS
        ),
        Err(_LinuxI2CError) => println!(
            "\x1b[91mFailed\x1b[0m => Channel  {}: MAX11645 ({:#02X})",
            RB_MAX11645_CHANNEL, RB_MAX11645_ADDRESS
        ),
    };
    ...
    let p3v5_ina226 = match p3v5_ina226 {
        Ok(_f32) => println!(
            "Passed => Channel  {}: 3.5V INA226 ({:#02X})",
            RB_P3V5_INA226_CHANNEL, RB_P3V5_INA226_ADDRESS
        ),
        Err(_LinuxI2CError) => println!(
            "\x1b[91mFailed\x1b[0m => Channel  {}: 3.5V INA226 ({:#02X})",
            RB_P3V5_INA226_CHANNEL, RB_P3V5_INA226_ADDRESS
        ),
    };
    ...
    println!(
        "Checking I2C Multiplexer 2 ({:#02X})",
        RB_PCA9548A_ADDRESS_2
    );
    ...
    let clk_tmp112 = match clk_tmp112 {
        Ok(_f32) => println!(
            "Passed => Channel  {}: CLK TMP112 ({:#02X})",
            RB_CLK_TMP112_CHANNEL, RB_CLK_TMP112_ADDRESS
        ),
        Err(_LinuxI2CError) => println!(
            "\x1b[91mFailed\x1b[0m => Channel  {}: CLK TMP112 ({:#02X})",
            RB_CLK_TMP112_CHANNEL, RB_CLK_TMP112_ADDRESS
        ),
    };
    ...
    let si5345b = match si5345b {
        Ok(_bool) => println!(
            "Passed => Channel  {}: SI5345 ({:#02X})",
            RB_SI5345B_CHANNEL, RB_SI5345B_ADDRESS
        ),
        Err(_LinuxI2CError) => println!(
            "\x1b[91mFailed\x1b[0m => Channel  {}: SI5345 ({:#02X})",
            RB_SI5345B_CHANNEL, RB_SI5345B_ADDRESS
        ),
    };
    ...
    let adc_dvdd_ina226 = match adc_dvdd_ina226 {
        Ok(_f32) => println!(
            "Passed => Channel  {}: ADC DVDD INA226 ({:#02X})",
            RB_ADC_DVDD_INA226_CHANNEL, RB_ADC_DVDD_INA226_ADDRESS
        ),
        Err(_LinuxI2CError) => println!(
            "\x1b[91mFailed\x1b[0m => Channel  {}: ADC DVDD INA226 ({:#02X})",
            RB_ADC_DVDD_INA226_CHANNEL, RB_ADC_DVDD_INA226_ADDRESS
        ),
    };
    ...
    let adc_tmp112 = match adc_tmp112 {
        Ok(_f32) => println!(
            "Passed => Channel  {}: ADC TMP112 ({:#02X})",
            RB_ADC_TMP112_CHANNEL, RB_ADC_TMP112_ADDRESS
        ),
        Err(_LinuxI2CError) => println!(
            "\x1b[91mFailed\x1b[0m => Channel  {}: ADC TMP112 ({:#02X})",
            RB_ADC_TMP112_CHANNEL, RB_ADC_TMP112_ADDRESS
        ),
    };
    ...
    let adc_avdd_ina226 = match adc_avdd_ina226 {
        Ok(_f32) => println!(
            "Passed => Channel  {}: ADC AVDD INA226 ({:#02X})",
            RB_ADC_AVDD_INA226_CHANNEL, RB_ADC_AVDD_INA226_ADDRESS
        ),
        Err(_LinuxI2CError) => println!(
            "\x1b[91mFailed\x1b[0m => Channel  {}: ADC AVDD INA226 ({:#02X})",
            RB_ADC_AVDD_INA226_CHANNEL, RB_ADC_AVDD_INA226_ADDRESS
        ),
    };
    ...
    let drs_avdd_ina226 = match drs_avdd_ina226 {
        Ok(_f32) => println!(
            "Passed => Channel  {}: DRS4 AVDD INA226 ({:#02X})",
            RB_DRS_AVDD_INA226_CHANNEL, RB_DRS_AVDD_INA226_ADDRESS
        ),
        Err(_LinuxI2CError) => println!(
            "\x1b[91mFailed\x1b[0m => Channel  {}: DRS4 AVDD INA226 ({:#02X})",
            RB_DRS_AVDD_INA226_CHANNEL, RB_DRS_AVDD_INA226_ADDRESS
        ),
    };
    ...
    let cy8c9560a = match cy8c9560a {
        Ok(_u8) => println!(
            "Passed => Channel  {}: CY8C9560A ({:#02X})",
            RB_CY8C9560A_CHANNEL, RB_CY8C9560A_ADDRESS
        ),
        Err(_LinuxI2CError) => println!(
            "\x1b[91mFailed\x1b[0m => Channel  {}: CY8C9560A ({:#02X})",
            RB_CY8C9560A_CHANNEL, RB_CY8C9560A_ADDRESS
        ),
    };
