...
async fn rb_matching() {
    ...
    for (i, (rb1_id, rb2_id)) in rat_config
        .rb1_id_arr
        .iter()
        .zip(rat_config.rb2_id_arr.iter())
        .enumerate()
    {
        if rb1_id == &rb_name {
            rb_id = *rb1_id;
            ...
            rb_influxdb
                .rb_influxdb_write(&rat_id_str, test.to_string())
                .await;

            let ltb_influxdb = ltb_influxdb::LTBInfluxDB::new(ltb_bucket);
            ltb_influxdb
                .ltb_influxdb_write(&rat_id_str, test.to_string())
                .await;
            ...
            pb_influxdb
                .pb_influxdb_write(&rat_id_str, test.to_string())
                .await;

            let preamp_influxdb = preamp_influxdb::PreampInfluxDB::new(preamp_bucket);
            preamp_influxdb
                .preamp_influxdb_write(&rat_id_str, test.to_string())
                .await;
            break;
        }
    }
