...
impl PreampInfluxDB {
    ...
    pub async fn preamp_influxdb_write(&self, rat_id: &str, test: String) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let mut preamp_temp_points = Self::preamp_temp_influxdb(&hostname, &rat_id, &test).expect("cannot get Preamp temperature InfluxDB data points");
        let mut preamp_bias_points = Self::preamp_bias_influxdb(&hostname, &rat_id, &test).expect("cannot get Preamp bias InfluxDB data points");
    ...
    fn preamp_temp_influxdb(hostname: &str, rat_id: &str, test: &str) -> Result<Vec<DataPoint>, Box<dyn std::error::Error>> {
    ...
    fn preamp_bias_influxdb(hostname: &str, rat_id: &str, test: &str) -> Result<Vec<DataPoint>, Box<dyn std::error::Error>> {
