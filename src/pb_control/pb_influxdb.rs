...
impl PBInfluxDB {
    ...
    pub async fn pb_influxdb_write(
        &self,
        rat_id: &str,
        test: String,
        ...
        let mut pb_temp_points = Self::pb_temp_influxdb(&hostname, &rat_id, &test)
            .expect("cannot get PB temperature InfluxDB data points");
        let mut pb_vcp_points = Self::pb_vcp_influxdb(&hostname, &rat_id, &test)
            .expect("cannot get PB VCP InfluxDB data points");
